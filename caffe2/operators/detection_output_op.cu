...
bool DetectionOutputOp<float, CUDAContext>::RunOnDevice() {
  ...
  const bool clip_bbox = false;
	...
	DecodeBBoxesKernel<float> <<<CAFFE_GET_BLOCKS(loc_count),
								...
								context_.cuda_stream()>>> (loc_count, loc_data, prior_data, code_type_, 
	...
	if (!share_location_) {
		...
		PermuteDataKernel<float> <<<CAFFE_GET_BLOCKS(loc_count),
									CAFFE_CUDA_NUM_THREADS, 0,
								    context_.cuda_stream()>>>(loc_count, bbox_data, num_loc_classes_,
								    num_priors_, 4, bbox_permute_data);
	...
	PermuteDataKernel<float> <<<CAFFE_GET_BLOCKS(conf_count),
								CAFFE_CUDA_NUM_THREADS, 0,
								context_.cuda_stream()>>>(conf_count, conf_data, num_classes_,
							    num_priors_, 1, conf_permute_data);
