...
template<>
void SampleBBox(const BatchSampler& sampler, NormalizedBBox* sampled_bbox,CUDAContext* context) {
  // Get random scale.
...
//  scale = generate_rand(sampler.min_scale(),sampler.max_scale()); 
  math::RandUniform<float,CUDAContext>(1, sampler.min_scale , sampler.max_scale , &scale , context);  
  // scale 0.3 ~ 1.0
  ...
  //    &aspect_ratio);
  math::RandUniform<float,CUDAContext>(1, sampler.min_aspect_ratio, sampler.max_aspect_ratio,
...
//  aspect_ratio = generate_rand(sampler.min_aspect_ratio(),sampler.max_aspect_ratio());
  ...
  float w_off, h_off;
  math::RandUniform<float,CUDAContext>(1,0.f,1 - bbox_width,&w_off,context);
  math::RandUniform<float,CUDAContext>(1,0.f,1 - bbox_height,&h_off,context);
...
void GenerateSamples(const NormalizedBBox& source_bbox,
                     ...
                     vector<NormalizedBBox>* sampled_bboxes,CUDAContext* context) {
  int found = 0;
